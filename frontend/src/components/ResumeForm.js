...
=======
  ...
  const degreeRank = (degree = '') => {
    ...
    if (/\b(BA|BS|BACHELOR)\b/.test(normalized)) return 2;
    if (/\b(MA|MS|MBA|MASTER)\b/.test(normalized)) return 3;
    if (/\b(PHD|PH D|DOCTOR|DOCTORATE)\b/.test(normalized)) return 4;
